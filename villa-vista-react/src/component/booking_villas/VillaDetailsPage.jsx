...
const VillaDetailsPage = () => {
  ...
  const { villaId } = useParams(); // Get room ID from URL parameters
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setVillaDetails(response.villa);
    ...
    fetchData();
  }, [villaId]); // Re-run effect when roomId changes
  ...
  const handleConfirmBooking = async () => {
    ...
    const villaPricePerNight = villaDetails.villaPrice;
    const totalPrice = villaPricePerNight * totalDays;
  ...
  const acceptBooking = async () => {
    try {
      ...
      // Make booking
      const response = await ApiService.bookVilla(villaId, userId, booking);
      if (response.statusCode === 200) {
        ...
        setTimeout(() => {
          ...
          navigate('/villa'); // Navigate to rooms
        }, 10000);
      }
    } catch (error) {
  ...
  if (isLoading) {
    return <p className='villa-detail-loading'>Loading villa details...</p>;
  }

  if (error) {
    return <p className='villa-detail-loading'>{error}</p>;
  }

  if (!villaDetails) {
    return <p className='villa-detail-loading'>Villa not found.</p>;
  }

  const { villaType, villaPrice, villaPhotoUrl, description, bookings } = villaDetails;

  return (
    <div className="villa-details-booking">
      {showMessage && (
      ...
      <h2>Villa Details</h2>
      <br />
      <img src={villaPhotoUrl} alt={villaType} className="villa-details-image" />
      <div className="villa-details-info">
        <h3>{villaType}</h3>
        <p>Price: ${villaPrice} / night</p>
        <p>{description}</p>
      </div>
